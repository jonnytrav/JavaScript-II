...
runners.filter(function(element) {
    if(element.shirt_size.includes("L")) {
...
let totalDonation = ticketPriceTotal.reduce(function(acc, currentIndex) {
...
// Problem 1
//Everyone who cannot get a large shirt has withdrawn from the race. Log the new array without the large shirt runners.
// runners.filter(function(element){
//     if(element.shirt_size.includes("L")) {
//         updatedRunners.push(runners.splice(runners["id"-1], 1))
//     }
// })
let updatedRunners = [];
runners.filter(function(value){
    if(value.shirt_size.includes(!"L")) {
        updatedRunners.push(value);
    }
})
console.log(updatedRunners);

// Problem 2
//The director has sobered up a little bit but is too tired to rewrite the names. Return a list with all first names, lowercased.
let runnersList = [];
runners.map(function(element) {
    runnersList.push(element.first_name.toLowerCase())
})
console.log(runnersList);

// Problem 3
//The runners are going to be lined up at the start in alphabetical order by last name. Log the correct list for the director.
let runnersOrder = [];
runners.map(function(element) {
    runnersOrder.push(element["last_name"])
})
runnersOrder.sort();
console.log(runnersOrder);